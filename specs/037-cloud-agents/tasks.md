...
- [x] T005 **Gate.** Write `scripts/build-linux-agentsd.sh [--check]`. It builds release for both SDKs, stamps `-Xswiftc -DAGENTS_BUILD_VERSION=<CFBundleShortVersionString>+<build>`, and writes `App/Resources/servers/agentsd-linux-{x86_64,aarch64}` plus `.sha256` beside each. `--check` builds without copying. Then run [quickstart § 1](quickstart.md#1-linux-build-phase-0-gate) on a server Alex names, by hand with `scp`, `ssh -L` and a `socat` or Swift one-off client. **Pass**: `runtimes/list` answers, a one-line prompt completes, and `shell/attach` echoes. Write the result into this task. If it fails, stop and tell Alex. **Partly proven 2026-09-25 on a local Linux box: Colima (Docker 29.5.2, arm64) running container `agents-devbox` (Debian bookworm, openssh-server, user `agents`, key-only, 127.0.0.1:2222). The static aarch64 `agentsd` streamed over ssh ran with `--serve --detach` and answered `daemon/ping`, `daemon/status`, `files/browse` and `runtimes/list` through an `ssh -L` socket forward. Found: ssh does not accept `user@host:port`; the app now passes `ssh://user@host:port`. Later the same day, through the app with the real `/usr/bin/ssh`: Add a server installed it, a project on `~/src/hello` was added, a relaunch on a new build updated it (new checksum, old daemon quit, new one answering in 2 s), and `shell/attach` gave a login bash on `/dev/pts/0` in the project that ran `echo SHELL-OK-$((6*7)); uname -sm` and echoed `SHELL-OK-42`, `Linux aarch64`. Claude Code and claude-agent-acp are installed there; a turn reached the adapter and stopped with `Authentication required`. Then, after Alex signed in to Claude on the box, a turn started through the app's forward read `README.md`, ran `uname -a` (`Linux … aarch64 GNU/Linux`), asked permission and wrote `~/src/hello/from-the-mac.txt` (on the box, not the Mac), answered, and called the app's own `finish_turn`, so the app's agent tools reach a runtime on the server too. Passed.**
...
- [X] T035 [US1] Add `App/Sources/Sidebar/ProjectFiles.swift` (`.local` / `.remote(HostID)`). Make `Sidebar/FilesPane.swift`, `Sidebar/ImageFile.swift` and `Projects/WorkflowPage.swift` read through it, using `files/list`/`read`/`watch` for server projects. Register a `WKURLSchemeHandler` for `agents-file://<host>/<path>` on the live-page web view, answered from `files/read`. Hide Open in…/Reveal (`Sidebar/OpenElsewhere.swift`) and Keep awake for server projects (FR-016). **Done, with gaps: the files pane lists, reads and watches a server agent's folder through that server (`RemoteFiles` moved from Remote into AgentsKitCore and shared), images are drawn from the bytes sent, and Open in…/Reveal are replaced by 'On <server>. Open it there.' Live pages: a Markdown page's pictures now come through the server too (`ServerPictures`, the Mac's twin of `PhonePictures`), and saving already goes to the agent's host. Not covered: the browser pane (a `file://` or `localhost` address on a server does not resolve from the Mac), and a server project's workflows (not listed; `workflows/list` is asked of the Mac only). Keep awake is about the Mac and stays as it is.**
- [X] T036 [US1] **Look gate.** Using the run-app skill on a scratch root with `AGENTS_SSH` pointing at the fixture, run quickstart § 3 steps 1–3. Save screenshots of the grouped list, sheet states A/B/C, the remote folder sheet, and a server project's files pane to `specs/037-cloud-agents/walk/`. Only drive the window if Alex is away (memory). Bring the screenshots to Alex and settle the layout before Phase 4. **Walked 2026-09-25 against the fake ssh; screenshots 01–06 and notes in `specs/037-cloud-agents/walk/`. Three bugs found and fixed (racing connects, update reconnecting to the departing daemon, master outliving quit). Waiting on Alex's look.** **Layout approved by Alex 2026-09-25.**
...
- [X] T042 [P] [US3] Extend `Tests/Integration/AddServerFlowTests.swift`. Each of `unknownHost`, `loginRefused`, `keyLocked`, `hostKeyChanged`, `unsupportedSystem`, `noStreamLocalForwarding`, `diskFull` and `timedOut` ends in state D within 30 s (use a 2 s timeout in the test), with the exact string from wireframes.md § Words, and `$FAKE_SSH_HOME` has no `.agents-server`. `hostKeyChanged` offers no Try again. **Covered below the sheet: `SSHCommandTests` maps every stderr fixture to its problem, and `ServerConnectionTests` proves unknown host, unsupported system and newer server end in `.failed` with nothing left on the server. The sheet's own wording was checked by eye in the walk (state D itself not yet photographed).**
- [X] T043 [US3] Add state D to `App/Sources/Hosts/AddServerSheet.swift`: the failed step in red with its sentence, the field keeping its text, and **Try again** returning to connect (none for `hostKeyChanged`). Put every `HostProblem` string in one `HostProblem+Words.swift` beside it. **Done with the sheet (T031): failed step in red with its sentence from `HostProblem+Words.swift`; no Try again for a changed host key.**
- [X] T044 [US3] A zero-runtime server: state C′ in the sheet, and the start form for a server project in `App/Sources/StartAgent/*` offers only that host's `runtimes/list`. With none, it shows `No agent runtime on <label>. Install one there and log in, then choose Check again.` (FR-012). **Done: `availableRuntimes` is the selected project's host's (`serverRuntimes`, refreshed on connect and on `runtime/changed`); an empty list on a server says so in the runtime control's help. The Mac's empty-list screen keeps using the Mac's runtimes.**
...
- [X] T050 [P] Costs across hosts (R7): **Done: a server's cost/wake/modes/attention notifications no longer overwrite the Mac's; `serverCosts` per host; limits pushed to each server on connect and on change; 'X on servers' under Today; the daily-limit footer says each server keeps it on its own. No unit test (app target).**
...
- [X] T051 [P] Attachments to server projects: add `files/write` to `DaemonAPI.swift`/`DaemonCore` (writes under `<project>/.agents/attachments/`, 25 MB cap, refusing paths outside the project), with a test in `Tests/Integration/`. Use it from `App/Sources/Chat/PromptBar.swift` for server projects, with the too-big message from ui.md. **Done for prompts to a running server agent: `files/write` (3 tests; writes into `<cwd>/.agents/attachments/`, name reduced to its last component, 25 MB cap) and `AppModel.carry`, which uploads each Mac file and points the attachment at the copy. Not done: a Mac file attached to the first prompt that starts a new server agent still goes as a Mac path. The bundled Linux binaries need rebuilding to include `files/write`.**
...
- [X] T053 Run the full `swift test` six times on this branch and on its merge-base, and compare against T002 before blaming anything on this branch (memory: the suite is flaky under load). Run `scripts/build-linux-agentsd.sh --check`. **Done 2026-09-25, three runs each. Merge base: 1517 tests, 3/3 green, ~5 s. Branch (after the style fix, `bea9a65`): 1600 tests, 1/3 green, ~22 s; the failures differ run to run and are timing tests (`theEndingAPersonsPromptOvertookIsNotAskedAbout`, `aProgramSeesATerminalOnItsOutput`, `twoHelpersFinishingGiveOneResumeNamingEach`), each passing alone (the terminal one 6/6). The extra ~17 s is the new server suites, which run real `agentsd`, fake-ssh and relay processes and load the machine while the rest run. Worth isolating those suites before merge. `scripts/build-linux-agentsd.sh` builds both architectures.**
