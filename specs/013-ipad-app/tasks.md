...
- [X] T064 [P] [US2] Show the project's cost total on `Remote/Sources/Projects/ProjectPageView.swift` and the grand total on a new `Remote/Sources/Projects/TotalsView.swift`, matching feature 012 on the Mac (parity.md). Reported, never estimated.
- [X] T065 [P] [US2] *(already true — verified 2026-09-19: `AgentCard.ending` reads `EndedReason.summary` and `StateLine` handles `.costLimit`, both the same shared words the Mac uses. One gap noted, shared with the Mac: `report?.message` wins over the ending reason, so a cost-limited agent that reported on its work shows the report. Not diverged from here — it is one change to `AgentRow` and `AgentCard` together.)* Show that a cost limit stopped an agent, in `Remote/Sources/Projects/AgentCard.swift` and `Remote/Sources/Chat/RemoteChatView.swift`, matching feature 010. **Showing, not setting** — changing a limit stays at the Mac (spec, Out of scope).
- [X] T066 [P] [US2] *(already true — verified 2026-09-19: `AgentCard` calls `model.isComingBack`, `StatusIcon` takes `isComingBack`, and `ComingBackLine` is at the foot of the conversation. All through the shared `AgentsModel`, which is what the task asked for.)* Show the resuming / coming-back state from feature 011 in `Remote/Sources/Projects/AgentCard.swift`, using the shared `AgentsModel.isComingBack(_:)` so the Mac and the iPad cannot disagree.
- [X] T066a [US2] **Added 2026-09-19, and the reason it was missing.** Create the conversation's prompt bar in `Remote/Sources/Chat/PromptBar.swift` — a field, a send, and `agents/prompt` — for FR-025. It was never a task because `contracts/parity.md` recorded it as built, which it was not: there is no `TextField` anywhere in `Remote/Sources/`. T067 and T069 both attach to it and neither could be done without it. Deliberately much less than the Mac's bar: the runtime, mode, model, effort and permission controls belong to T072's start screen, not over a conversation on a phone.
- [X] T067 [P] [US2] Add the runtime's slash commands while typing to `Remote/Sources/Chat/RemoteChatView.swift`'s prompt bar, mirroring `App/Sources/Chat/CommandList.swift` (parity.md). The list is the runtime's, not ours.
- [X] T068 [P] [US2] Add the jump-to-the-live-end control to `Remote/Sources/Chat/RemoteChatView.swift`, mirroring `App/Sources/Chat/JumpToEnd.swift`: shown only when it would do something.
- [X] T069 [P] [US2] Add suggested next prompts to `Remote/Sources/Chat/RemoteChatView.swift` via `agents/suggestPrompts`, as the Mac shows them (parity.md).
- [X] T070 [US2] Size the first transcript page to the screen rather than a constant, in `Remote/Sources/RemoteModel.swift`. An iPad shows two to three times a phone's lines, and a constant page means the person watches it fetch twice before they have read anything (research §9, SC-007).
...
- [X] T072 [P] [US3] **Carried by 029** (`specs/029-start-agents-on-iphone`, 2026-09-24): New agent is in `ProjectPageView`'s toolbar and opens `Remote/Sources/StartAgent/StartAgentView.swift`, a form sheet on the iPad, with the runtimes and every choice the Mac's runtime advertises. Extra folders, MCP servers and extra arguments stay on the Mac by 029's decision. Not seen on an iPad yet. Original wording: Start an agent in an existing project from `Remote/Sources/Projects/ProjectPageView.swift`, choosing from the runtimes the Mac has via `runtimes/list` and `agents/options`, with the mode, model, effort and permission controls the Mac offers (FR-026, parity.md).
...
- [ ] T074 [P] [US3] **Half carried by 029**: the start sheet attaches pictures and text files by value, through `PhoneAttachment` (AgentsKitCore) and `AttachButton`/`AttachmentStrip` in `Remote/Sources/StartAgent/PhoneAttachments.swift`. What's left is putting the same two views on the conversation's `PromptBar` and sending attachments with `agents/prompt`. Original wording: Attach a file or image to a prompt from the iPad's photo library or Files in `Remote/Sources/Chat/AttachmentStrip.swift` (new file), refused before sending when the runtime cannot take them — the same refusal the Mac gives, for the same reason (FR-025).
