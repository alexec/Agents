...
- [ ] T006 [P] Add `alreadyAnswered = -32018` and `noSuchDevice = -32019` to `DaemonAPI.Failure` in `Packages/AgentsKit/Sources/AgentsKitCore/Daemon/DaemonAPI.swift`. **Not** 005's `-32013`/`-32014`, which are `projectHasLiveAgents` and `noSuchWorkflow`. Pick by reading the file: it already contains a collision at `-32010`. *(research §7)*
- [ ] T007 [P] Add `devices` to `StoreLocations` in `Packages/AgentsKit/Sources/AgentsKit/Store/StoreLocations.swift`, returning `root.appendingPathComponent("devices.json")`, beside `projects.json`. *(005 T005)*
