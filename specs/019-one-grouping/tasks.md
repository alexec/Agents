...
- [X] T009 [US1] Add `public func counts(in folder: URL?) -> [AgentGroup: Int]` to `Packages/AgentsKit/Sources/AgentsKitCore/Client/AgentsModel.swift` beside `agents(in:group:)`, using the same standardised-folder filter and `group(of:)`. Comment that this is what FR-009 means by the count being completed by something that knows: the daemon's `ProjectSummary.counts` are computed without eyes, and this window has them (FR-006, FR-007)
- [X] T010 [US2] Delete `wantsEyes(in:)` from `AgentsModel.swift:306` and its forwarder at `App/Sources/AppModel.swift:227`. Its replacement is `counts(in:)[.needsAttention] > 0`, which consults state through the grouping — the check it replaces never did, which is how a stopped agent went on wanting eyes (FR-011, FR-012)
- [X] T011 [US1] In `App/Sources/Projects/ProjectRow.swift:57` and `:66–67` read `model.counts(in: summary.folder)` instead of `summary.counts`, and make `needsPerson` exactly `(counts[.needsAttention] ?? 0) > 0` (FR-008). Rewrite the comment above it: it used to explain two ways to want somebody, one the daemon counts and one only the window knows; now there is one grouping and this window asks it
- [X] T012 [US1] Add `func counts(in folder: URL?) -> [AgentGroup: Int]` to `App/Sources/AppModel.swift` beside `agents(in:group:)` (`:223`), forwarding to `work.counts(in:)`
...
- [X] T020 [P] Extend `noHeadingWasAddedRenamedOrRemoved` in `AgentGroupTests.swift` only if it does not already pin `AgentGroup.live`'s order and titles; otherwise leave it and say so in the commit (FR-022)
- [X] T021 Run `swift test --package-path Packages/AgentsKit`, `xcodebuild -scheme Agents -configuration Debug -skipPackagePluginValidation -skipMacroValidation build` and the same for `-scheme Remote -destination 'generic/platform=iOS Simulator'`, all green
...
- **T001–T021 done in one pass**; package suite **952 tests in 106 suites** (one pre-existing flake, `stoppingAnAgentBeforeItIsPickedUpWithdrawsIt`), `Agents` and `Remote` both **BUILD SUCCEEDED**. **T022 — the walk — is open and is Alex's**; check 5 needs the phone.
- Removing the defaults broke ten call sites, not the nine the plan counted: `AgentBirthTests` reads `.group.title` as well as `.group ==`. The compiler found it, which is the mechanism the plan relies on.
- `Agent.group` became `Agent.group(wantsEyes:)` rather than being deleted outright, so the daemon, the preview and the daemon-side tests have one honest spelling — `group(wantsEyes: false)` — and the scan can name one file. `AgentGroup(for:` is now called in exactly one place, and `OneGroupingTests` holds it.
- T020: `noHeadingWasAddedRenamedOrRemoved` already pinned the titles and `liveOrder` the order; both were kept in the rewritten `AgentGroupTests`, and a third test, `theFlagMovesNothingButARunningAgent`, was added beyond the plan: `outcomeAsked` must change nothing for any state but `running`, and nothing else in the suite said so.
- The phone is unchanged and correct: its `AgentsModel` never fills `filesToShow`, so its `group(of:)` and the daemon's counts agree, and SC-005 holds for every fact the daemon has. The Mac's `ProjectRow` no longer reads `summary.counts` at all.
