...
- [X] T046 [P] [US3] Add `writingASettingChangesTheFileAndNothingElse`, `theWindowHearsAboutItWithoutWaitingForTheWatcher`, `aSettingOnAReadOnlyFileIsRefusedAndNotHeld` and `rememberedOptionsStartsNothing` to `Packages/AgentsKit/Tests/AgentsKitTests/Integration/WorkflowSettingsFlowTests.swift`
...
- [X] T050 [P] [US4] Extend `Packages/AgentsKit/Tests/AgentsKitTests/Integration/WorkflowToolTests.swift` with a write carrying `permission-mode:` and assert the reply names the mode in words — not as a quoted line of YAML
...
- `WorkflowToolTests` flakes in filtered runs (`--filter WorkflowToolTests`) on the token-drop race its own `keepingAlive` helper describes — 2 of 3 runs at clean HEAD before any of this. The full suite is not affected.
