...
- Q: What can you do with workflows on the project page? → A: List with trigger and next fire, Run now, archive. (Originally "pause"; pause and archive turned out to be one thing said twice — see FR-024.) No separate run history — every run starts or resumes an agent, and the agent list already shows those.
...
5. **Given** a running or scheduled workflow, **When** the reader archives it, **Then** it stops firing, its row says so under *Archived*, and the workflow file on disk is unchanged.
6. **Given** several workflows in a project, **When** the reader archives them, **Then** none of them fire until they are restored, and each row says which.
...
**Why this priority**: The schedule is useful, but reacting to agents is what makes this an *agentic* workflow rather than a cron job. It is second because it depends on the whole of story 1 — a workflow that cannot be listed, held, or run by hand is not safe to hook to something that fires as often as an agent finishing.
...
- **A renamed file.** Identity comes from the file name, so renaming a workflow is indistinguishable from deleting one and creating another: the archive flag and the `standing` agent do not follow it.
...
- **FR-004**: A workflow MUST be identified by its file name, and that identity MUST be what the system uses to remember which agent is a workflow's standing agent and whether it is archived.
...
- **FR-012**: Every workflow MUST be runnable on demand from the project page, whatever its triggers, and this MUST be available even for one that has no trigger the system understands or is over a ceiling. An archived workflow is the exception: it offers Restore and nothing else.
...
- **FR-024**: *Superseded by FR-031a.* The reader MUST be able to stop a workflow running without modifying its file, and archiving is that — one control rather than two. A separate pause was built and removed: it and archiving both meant "do not run this", both were one tap to undo, and the only difference was that a paused row sat in the list looking live. There is no project-wide switch either; archiving says on each row what went quiet.
- **FR-025**: An archived workflow MUST refuse every fire, including one asked for by hand, and MUST say that being archived is the reason.
- **FR-026**: Every refused fire MUST be recorded with its reason. The reasons MUST include at least: the chain-depth limit was reached; a run is still in flight; the workflow is archived; a ceiling is full; the metadata could not be read; the trigger is not supported; the agent to resume was unavailable; the scheduled time was missed while the app was not running; the project folder is unavailable.
...
- **FR-035**: *Superseded.* Creating, changing or removing a workflow MUST NOT require approval, and MUST take effect whether or not a window is open. The original rule — a confirmation the daemon raised and blocked the write on — is replaced by FR-031a: the person's say happens after the fact, on the project page, where there is something to look at. Asking first made the write fail whenever nobody was there to answer, which turned the feature off at exactly the hours it was for.
- **FR-036**: What the tool tells the agent after a write MUST state, in plain language, what will cause the workflow to run and which agent will run it, and MUST say where the person can see it — not merely that a file was written.
- **FR-037**: The tool MUST refuse to read or write anything outside the workflow folder of the project the agent is working in.
- **FR-038**: A workflow created through the tool MUST need no further step to become live.
- **FR-039**: A write to a workflow the person has archived MUST leave it archived, and the tool MUST say so.
- **FR-040**: The tool MUST refuse to create a new workflow when either ceiling is reached — naming the three in the way for a full project, and saying the remedy may be in another project for a full machine — and MUST still allow changing a workflow that already exists. An agent that can write without asking must not be able to fill a project, and ten projects of three must not be thirty. The one thing a person can say about a workflow they did not ask for is not undone by the agent it was aimed at.
...
- **Workflow state**: What the app remembers about a workflow that does not belong in the file — whether it is archived, which agent is its standing agent, and the outcome of its last fire. Keyed by the workflow's file name, kept out of the repository.
...
- Whether a workflow is archived, and the identity of its standing agent, are held by the app rather than written back into the file. Writing them to disk would put the app's own bookkeeping into the repository's history, where nobody wants to review it.
