...
- [X] T062 Record the per-runtime adoption rate from T061 in `specs/014-agent-outcomes/research.md` as a new section, against SC-007's 90% target — a runtime that will not call the tool is a finding to write down, not a failure of the design. **Run 2026-09-19.** R11 now holds the measurement: 1 of 3 runtimes called the tool unprompted (33% against SC-007's 90%). grok reported `needs_answer` with the question as its message; claude ended in `waitingOnUser`, which lands in the same group by a route older than 014; copilot said nothing, was asked once, and said nothing again — the ask-once bound holding in a live run rather than in a fake.
