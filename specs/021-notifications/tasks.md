...
- [X] T026 [P] [US1] Create `App/Sources/Notifications/PresenceReporter.swift`: reports `presence/report` when the app becomes or resigns active, when the selected conversation changes, and when input is seen after a quiet spell. **A report is sent on a change, not on a timer** — a surface already `active` with the same `watching` sends nothing, so typing does not send one per keystroke
- [X] T027 [US1] Create `App/Sources/Notifications/MacNotifier.swift`: requests authorisation, posts a `UNNotificationRequest` whose identifier is the need's id, and withdraws with `removeDeliveredNotifications(withIdentifiers:)`. It applies the idempotent rule from the contract and nothing else — `to == me and not showing it → show it, with sound iff alert`; `to != me and showing it → withdraw`; `need == nil and showing it → withdraw`; otherwise nothing. It **decides nothing**; that is FR-012 expressed as a file boundary rather than as a comment
- [X] T028 [US1] Build the banner's three lines in `MacNotifier` from `Need.headline` alone — `h1` the project, `h2` the agent, `h3` what is wanted — never from anything the Mac recomputes locally. An empty or enormous project or agent name must still identify which agent it is, **truncated rather than blank** (spec edge case)
- [X] T029 [US1] Handle the notification response in `MacNotifier`: opening it selects that agent's conversation with the question in front of the person and answerable there (FR-019). It must not answer anything itself
- [X] T030 [US1] Wire `PresenceReporter` and `MacNotifier` into `App/Sources/AppModel.swift` against the model it already has. Swiping a banner away must **not** mark the need met — there is no "dismissed" in the lifecycle; the agent is still blocked and the project row still says so, and conflating the two would be the app lying about the work
...
- [X] T040 [US4] In `DaemonCore+Attention.swift`, broadcast `attention/changed` with `need == nil` when a need is met — answered anywhere, or the agent stopped or archived (FR-016). Broadcast to **every** connection and not only to `to`: that is what lets the losers withdraw, and it is why FR-016 needs no second message. A withdrawal names only the need's id, which identifies nothing about the work
- [X] T041 [US4] Implement the move in `DaemonCore+Attention.swift`: when a need is outstanding and the person changes surface, one `attention/changed` with the new `to` — not a withdrawal and a delivery that could cross (FR-017). The whole resolved fact, in one notification, per need
- [X] T042 [P] [US4] Add to `AttentionTests.swift`: a need answered on connection A produces `need == nil` on connections B and C within the same broadcast; a need whose agent is stopped or archived does the same (US4 scenarios 1–3)
...
- [X] T047 [P] [US2] Create `Remote/Sources/Notifications/PresenceReporter.swift`: reports `presence/report` on `scenePhase` changes and on selection changes. `active` on a device means **foreground and unlocked** — a phone in a pocket with the app foregrounded is not a place a person is looking
- [X] T048 [US2] Create `Remote/Sources/Notifications/DeviceNotifier.swift`: the same two jobs as `MacNotifier` and the same idempotent rule, on iOS. It shows and withdraws a **local** notification when `attention/changed` names it; it decides nothing
- [X] T049 [US2] Ask for notification permission on the device where the person can see why (FR-024), and report the resulting authorisation status as `mayNotify` on every `presence/report` (FR-023)
- [X] T050 [US2] Wire both into `Remote/Sources/RemoteModel.swift`, and prime from `attention/pending` on connect so a remote that was not listening is put right
- [X] T051 [US2] Give the existing bridge connection a device identity so `presence/report` from a remote resolves to `.device(UUID)` rather than `notASurface` (T018's other half). Until Phase 7 stores real devices, the identity may come from the remote's existing handshake — but it must come from the **connection**, never from the report's parameters
...
- [X] T054 [P] [US2] Add to `AttentionTests.swift`: two device reports arriving within the same instant resolve by the daemon's `heardAt` and not by anything either device claimed; on an exact tie the iPhone wins (spec edge cases)
...
- [x] T059 [P] [US5] Create `Packages/AgentsKit/Sources/AgentsKitCore/Remote/DeviceKey.swift`: P256, Secure-Enclave-backed where available, with the private key in the **device's** keychain — which is the device's, not ours, and is why this does not break the rule about storing nothing outside the daemon's root
- [x] T060 [P] [US5] Create `Packages/AgentsKit/Sources/AgentsKitCore/Remote/Envelope.swift`: HPKE seal to a device's public key, with `Headline` sealed inside. Nothing legible goes to a device that is not `to` (FR-022)
...
- [x] T063 [US5] Implement revoke as **deletion**, not a flag: the record goes and that device's mailbox is emptied, so anything already waiting for it is discarded unread (FR-021). Deletion is what makes a device unable to read anything rather than a flag somebody must remember to check
- [x] T064 [US5] Wire the four methods into `DaemonCore+Dispatch.swift`, and make `Routing` consult the real `DeviceStore` rather than whatever stand-in Phase 6 used
- [x] T065 [P] [US5] Create `Packages/AgentsKit/Sources/AgentsKitCore/Remote/Mailbox.swift`: the protocol, plus a fake for tests
- [x] T066 [US5] Create `Packages/AgentsKit/Sources/AgentsKitCore/Remote/CloudKitMailbox.swift`: the real one, in the private database, with `CKSubscription.NotificationInfo.collapseIDKey` set to a **stable id per need** so a stale banner is overwritten by the next one for the same need rather than stacking (research §8)
- [x] T067 [US5] Create `Packages/AgentsKit/Sources/AgentsKitCore/Remote/MailboxTransport.swift`: the third `LineTransport`, beside the existing two, so the bridge carries `attention/changed` to a device that is not on the LAN
- [x] T068 [US5] Add the `RemoteNotify` notification service extension target to `project.yml` and create `RemoteNotify/Sources/NotificationService.swift`: it decrypts the envelope and assembles the banner's three words **on the device**, which is what a notification service extension is for. Sending the words in plaintext is FR-022 gone; fetching them after the person taps means a banner that says "An agent needs you" every time, which the quickstart already names as a failure
- [x] T069 [US5] Add the App Group shared between `Remote` and `RemoteNotify` in `project.yml`, plus `aps-environment` and the iCloud entitlements on both
- [x] T070 [P] [US5] Create `Remote/Sources/Devices/PairingView.swift`: announce this device, show that it is waiting for approval, and say plainly what it is waiting for
- [x] T071 [P] [US5] Create `App/Sources/Settings/DevicesPane.swift`: the paired devices, each with its name, kind and **whether it is able to show notifications** (FR-023) — a device that has not been permitted says so here rather than appearing to work — and approve and revoke
- [x] T072 [P] [US5] Add to `AttentionTests.swift` against the fake mailbox: an unpaired device is sent nothing; a device that has announced and not been approved is sent nothing; a revoked device is sent nothing and its mailbox is empty (US5 scenarios 1–3, SC-007)
- [x] T073 [P] [US5] Add a unit test in `Packages/AgentsKit/Tests/AgentsKitTests/Unit/` asserting that what a mailbox carries is ciphertext: the project name, the agent name and the headline do **not** appear in the bytes (FR-022, US5 scenario 4). Assert the property directly against a constructed envelope — do not reach for a running system
- [x] T074 [US5] Walk quickstart C3, C5 and C6 on real hardware and record the results, checking C5 against the CloudKit dashboard as well as against the device
...
- [x] T076 [US1] Seal the `Headline` per device with `Envelope` before it leaves the Mac, truncating to the budget T004 of 005 measured, and degrading to the placeholder rather than cutting mid-ciphertext
- [x] T077 [US1] Make opening a pushed notification take the person to that agent's conversation in the Remote app, with what is wanted in front of them and **answerable there** with the same choices the Mac offers (FR-019, US1 scenario 3)
...
- **T043 is not held by a daemon test.** `FakeACPAgent` issues its client requests one at a time, so two needs cannot be outstanding on one agent through it; `NeedTests` holds the identity half (two questions are two ids) and `RoutingTests` the independence of decisions. A fake that can ask twice at once is the way to close it.
- **The fake asks again on every turn**, including the turn the app's own outcome question starts; two Slice A tests pin their assertions to the question's own id for that reason.
- **T026/T035**: "watching" is the selection while the app is active; `active` is `NSApp.isActive`; a report is sent on a change and on input after a quiet spell of `macIdle / 2`, never on a timer. A window on a locked Mac reports nothing new, and its last report ages past `macIdle` in the daemon, which is the same fact reached the other way round.
- **T031**: authorisation is requested the first time a need is routed to the Mac, in `MacNotifier.ensureAuthorised`, and a denial is remembered so the request is not repeated.

## Implementation Notes — Slice B, without the hardware (2026-09-20)

- **T047–T054 built; T055 — the walk on real hardware — is Alex's.** Package suite 996 tests (one pre-existing flake); `Agents` and `Remote` both build.
- **T051 is `surface/identify`.** The LAN bridge in `Bridge/Sources/main.swift` is a byte forwarder and knows nothing about who is on the far end, so a remote says which device it is once, right after connecting, and `DaemonServer` takes the identity from that request **before** handing it on — every later request on the connection is that device's, and `presence/report` still carries no identity of its own. It is the stand-in the task allowed: Phase 7's T035 makes the bridge refuse an unpaired device at accept, and T064 replaces the registry.
- **The registry is `DaemonCore.knownDevices`**: a device that has identified is treated as approved, with `mayNotify` from its own reports and `lastSeenAt` from its presence. Nothing is written down. Marked as the stand-in it is.
- **Local notifications on the device reach the person only while the app is running** — quickstart B4, the LAN link's known limit and the reason Phase 8 exists.
- **`xcodegen generate` is needed after adding a source file**; the project lists files explicitly.

## Implementation Notes — Slice C, up to the container (2026-09-20)

- **T056 passed as far as code can take it.** `agents-bridge` is `type: application` with `Bridge/Info.plist`, `Bridge/agents-bridge.entitlements` and an embedded team profile; `--spike` reached CloudKit as the bundle (`accountStatus == available`) and the server answered `"Bad Container" (5/1014)` — the container is not registered. **T057 is the portal step and it is Alex's**: `iCloud.com.alexecollins.agents`, assigned to the bridge, the Remote and `com.alexecollins.agents.remote.notify`. Re-run the spike after; research §7 has the record.
- **Built without the container: T058–T071, T073, T075–T077, T079.** Package suite 1011 tests; `agents-bridge`, `Agents` and `Remote` (with `RemoteNotify.appex` embedded) build. Commits `276ecb7`, `535b0a0`, and the one that carries this note.
- **Identifying no longer pairs.** `surface/identify` only names the connection; a device becomes one through `devices/announce` (with its key) and `devices/approve`. `Routing` sees `approvedDevices` from `DeviceStore`; the Phase 6 `knownDevices` stand-in is gone. The Remote announces on every connection and `PairingView` says at the top that it is waiting.
- **T064 keeps `mayNotify` on the record**, refreshed from the device's own reports, so the Devices pane can say a device cannot show notifications after a restart (FR-023).
- **The daemon has no `Mailbox` of its own**: `DaemonCore.mailbox` is `nil` in production and a sealed `MailboxItem` goes out as `mailbox/post` (a revocation as `mailbox/empty`) for the bridge's `MailboxTransport` to carry. A test hands in `FakeMailbox`. Every connection hears the broadcast and that is safe — a need id, a device id and ciphertext name nothing.
- **T067 deviates from its text**: `MailboxTransport` is not a `LineTransport`, because nothing comes back down it. It is a client of the daemon in the bridge (`Bridge/Sources/MailboxTransport.swift`), off unless `AGENTS_BRIDGE_NO_MAILBOX` is unset.
- **T075 posts on every delivery to a device**, not only when the device has no live LAN connection: the daemon does not know which device connections are live, and a duplicate banner is collapsed by the need's token on the device (`collapseIDKey`). Refine when the walk shows it matters.
- **Two subscriptions per device** (`loud-<id>`, `quiet-<id>`): `alert == 1` items push with mutable content for `RemoteNotify`; `alert == 0` items — silent moves and withdrawals — push content-available and `RemoteModel.receivedPush` shows or removes a local notification. The silent half is best effort by Apple's design, and `DeviceNotifier.sweep` is why that is tolerable.
- **T078 is met over the LAN only.** A pushed banner opens the conversation (`needToken` → agent) and the question is answerable there through the existing sheets when the device can reach the Mac. Answering with **no** LAN would need a reply record in the mailbox and the daemon to hear it; not built, and not in the spec's text.
- **T057 done 2026-09-21** — the container, both App IDs and the App Group were registered by Xcode from the entitlements; the spike passes. See research §7.
- **Approve and revoke are gone (Alex, 2026-09-21, during the C walk).** Announcing is pairing: `devices/announce` writes the record and the ladder sees every announced device; `devices/approve`, `devices/revoke`, `device/changed { gone }`, `mailbox/empty` and `Mailbox.empty` no longer exist, `Device.approvedAt` is gone, `PairingView` is gone, and the Devices pane is a list with no buttons. T062/T063/T071's text describes what was built and then removed; US5 scenarios 2–3 and FR-021 are not met and are not wanted. `AttentionTests` US5 now holds: unpaired sent nothing; announced is sent the sealed need and can open it; a met need is withdrawn from the mailbox; a new key under an old id is refused; pairing survives a restart.
- **C3, C4 walked 2026-09-21, pass; C6 in part; C5 struck** — see the quickstart. The walk found and fixed (`a0ce24b`): a need in an archived project; the tap crash (async delegate callback off the main thread); the tap stopping at the project; silent pushes re-showing dismissed banners; and 15 stale build bundles registered with LaunchServices on the Mac.
- **Open:** T080's C2 (ten cellular trials, Mac locked, worst for SC-001) and Phase 9., T072's live half is covered by `AttentionTests` US5 against the fake mailbox, and Phase 9 (README, scans, SC measurements) waits on the walks.
