...
- [X] T057 **Walked, and SC-001 holds.** Five real Claude chats against `/tmp/agents-restart-test`, all `running`, daemon `kill -9`ed. The log has the ordering the feature rests on: `marked 5 agent(s) stopped: their processes were gone` at 22:50:23, `listening on …/daemon.sock` the same second, first `picked agent … back up after the restart` at 22:50:28 — nothing marked live before the socket, nothing picked up before it. All five were back inside **37 seconds**, one at a time, `resuming` draining 3 → 2 → 1 → 0. Every one carried the break line and the bracketed restart words, and `restartPickUps` read exactly `1` on all five. The window showed `Coming back after a restart` on a row still filed under **Stopped**, which is T023 behaving as written. One correction to the method: the kill command this file gave was a pattern wide enough to match the *ordinary* daemon too — it has been narrowed to the throwaway root's own `daemon.lock` pid, because `agentsd` is the same binary under both roots
