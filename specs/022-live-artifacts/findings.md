...
| Runtime | Date | Slices |
|---|---|---|
| Claude | 2026-09-23/24 | A walked on screen; B, F measurements over the socket; C half walked |
...
| The page following whole-file writes (line diff) | seen 2026-09-24 | Four writes, four follows: the new section at the bottom scrolled into view, the rewritten summary kept the view at the top, the rewritten SVG pulled the view back to the picture. `walk/A-*.png`. | Works as specified. Whether following should be softer when the reader has scrolled away is still untested — the reader was not scrolled away. |
| Marks placed on the right passage | seen 2026-09-24 | 4/4 writes marked exactly the passages that changed, including the agent's unannounced knock-on edits to the summary and the Notes when it changed the table. A heading and its paragraph are two passages and two marks. | Right. The typed reveal with a caret and the tint together are not too much; the tint says *where*, the caret says *now*. |
...
| Passage-level editing as the shape of editing | half seen 2026-09-24 | A click on a rendered passage opened it as an editor once the passage was a `Button` (a tap gesture under text selection never fires). It opened one line high and did not take focus from the prompt bar; both fixed in code, neither yet walked. Typing, saving and the merge are still unseen. | The shape is right so far — click, and the paragraph is yours — but the walk of typing is still owed. |
| The sidebar as the page's home vs a window of its own | seen 2026-09-24 | At the default pane width the page reads as paper and the conversation beside it is what the agent is saying about the document; the two together are the point. Not tried at 900. | The sidebar is the right home for the PoC. A wider pane, not a window. |
...
| SVG images beside the document, drawn and marked on change | half measured 2026-09-23 | Asked for a report "with a graph of those five numbers" and nothing about how, Claude wrote `test-counts.svg` beside the document and referenced it as an image; Grok wrote `graph-grok.svg` and did the same (2/2). Both also put the numbers in a table. Seen 2026-09-24: the redraw was marked and followed. But the picture renders as a **black rectangle**: Claude's SVG "adapts to light and dark" with a CSS media query, and `NSImage` does not evaluate CSS, so the dark variant's background wins. `walk/E-1-*.png`. | Agents draw SVG unprompted. The renderer needs either a note in the description ("no CSS, no media queries") or a WebKit-backed SVG view; the cheaper is the sentence, and it is a finding for the real feature. |
