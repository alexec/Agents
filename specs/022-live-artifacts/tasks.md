...
*2026-09-23.* Code only; the walk with real runtimes (T055) is still to do. Two notes
from building it. T050's `ArtifactEdit` lives in two shapes: the daemon's, with a date,
in `DaemonCore+Artifacts.swift`, and `Briefing.ArtifactEdit`, without one, so the wording
