...
/// What an agent's row shows beside its title: one of four shapes, and no more.
///
...
/// the title. The shape only has to say which of four things is true.
///
...
public enum StatusShape: Hashable, Sendable {
    ...
    public init(state: AgentState, outcome: WorkOutcome?, isComingBack: Bool) {
        ...
        case .finished: self = outcome?.needsAPerson == true ? .needsYou : .done
