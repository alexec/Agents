...
/// what the person is doing right now is not a fact worth keeping, and which devices
/// are theirs is. A device that announces itself is paired — there is no approving and
/// no revoking (Alex, 2026-09-21): the LAN the announce arrives over is the person's
/// own, and the mailbox it is then reached through is their own iCloud account. One
/// record per `id`; a reinstalled device is a new id and a new key. The public key is
/// never rewritten under an id — a device that wants a new key is a new device.
///
...
/// chosen (FR-023). `wants` is deliberately not built: every paired device is eligible
...
public struct Device: Hashable, Sendable, Codable, Identifiable {
    ...
    public init(id: UUID, publicKey: Data = Data(), name: String, kind: Kind,
                announcedAt: Date, lastSeenAt: Date? = nil,
    ...
    /// Whether the ladder may choose this device at all: it has said it can show a
    /// notification. Choosing one that cannot would be choosing silence.
    public var isEligible: Bool { mayNotify == true }

    enum CodingKeys: String, CodingKey, CaseIterable {
        case id, publicKey, name, kind, announcedAt, lastSeenAt, mayNotify
    }
