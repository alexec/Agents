...
/// SHA-256 of some bytes, as lowercase hex, on the Mac and in the Linux `agentsd` alike.
///
/// For one purpose: saying a workflow file is the one the person approved. A weaker
/// checksum would not do, because the thing being kept out is an agent that edits the
/// file, and an agent can make a file to match a checksum. CryptoKit is not in the
/// static Linux build, so the algorithm is here, checked against the standard vectors.
